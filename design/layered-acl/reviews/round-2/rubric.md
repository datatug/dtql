...
## A2 implementation-plan review focus

Review current files 01–17, contracts and shared source evidence in this frozen input. A1 approved decisions/closure are context, not proof of correctness. Assess independently whether C1–C7 and package dependencies/interfaces/tests are executable without architectural invention. Specifically test exact evidence session lifecycle vs existing transactions, visibility/evidence distinction, sample ID/reducer/schema consistency, crash recovery sequence and C7 native/procedure gating/inclusion-exclusion masks. User examples are User_* only and all except sys_*. Native effects execution remains outside MVP; verify this is honest and failclosed. Avoid expanding into long-running transactions or a new auth service.

Use A2- or O2- finding IDs. Return a full but concise report (target <=3500 words), cite exact section/definition and concrete failure. Do not read any other current-round review or modify files. Review the same frozen material only; no external lookups or extra repository evidence. Source excerpt limitations must be stated rather than guessed. Structural fixture validation reported in packet was run; runtime semantic vectors are specified only. Return findings rather than patches or implementation.
